...
function binaryMetrics(probabilities, labels) {
  let tp = 0, tn = 0, fp = 0, fn = 0, brier = 0;
  probabilities.forEach((probability, index) => {
    const predicted = probability >= 0.5 ? 1 : 0, label = labels[index];
  ...
  return { balancedAccuracy: (recall + specificity) / 2, precision, recall, f1: 2 * precision * recall / Math.max(1e-9, precision + recall), brierScore: brier / labels.length, confusionMatrix: { tp, tn, fp, fn } };
}
...
function evaluate(model, rows) {
  ...
  const drought = [0, 1, 2].map((horizon) => binaryMetrics(predictions.map((item) => item.drought[horizon]), rows.map((row) => row.targets.drought[horizon])));
